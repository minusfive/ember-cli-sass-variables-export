...
const debug = require("debug")("export-sass-variables");
  ...
  switch (val.constructor.name) {
    ...
    case 'SassColor':
      if (val.getA() === 1) {
        ...
        output = `rgba(${parseInt(val.getR())}, ${parseInt(val.getG())}, ${parseInt(val.getB())}, ${parseInt(val.getA())})`;
      }
...
module.exports = (outputPath) => ({
  'export($fileName, $value:())': (fileName, value) => {
    ...
    fs.writeFileSync(path.join(outputPath, `${fileName.getValue()}.js`), `/* eslint-disable */
